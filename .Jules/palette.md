...
## 2026-05-19 - Keyboard accessibility on interactive divs
**Learning:** Custom interactive elements (like the div in MosaicTile) that use `onClick` do not natively support keyboard navigation. They lack focus states and don't respond to Enter or Space keys, which creates a critical barrier for keyboard and screen reader users.
**Action:** Always add `role="button"`, `tabIndex={0}`, and an `onKeyDown` handler for Enter and Space (calling `e.preventDefault()` to stop page scrolling on Space) to interactive `div` or `span` elements. Also, always add `focus-visible` utility classes for clear visual focus indication.
