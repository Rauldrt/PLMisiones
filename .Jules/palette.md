...
## 2026-06-06 - Focus styles for Radix UI triggers
**Learning:** When using custom elements (like `<button>`) inside Radix UI triggers (e.g., `PopoverTrigger`, `DialogTrigger`) with `asChild`, default keyboard focus styles are lost.
**Action:** Manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`) and appropriate border-radius classes to the custom trigger element to ensure keyboard accessibility.
