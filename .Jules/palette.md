...
## 2024-05-17 - Keyboard Focus on asChild Triggers
**Learning:** When using custom elements (like `<button>`) inside Radix UI triggers (e.g., `PopoverTrigger`, `DialogTrigger`) with `asChild`, default keyboard focus styles are lost. Using standard `focus:` utilities causes focus rings to appear on mouse clicks as well, which is often undesirable for visual components.
**Action:** Manually apply Tailwind CSS `focus-visible:` utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) to the custom trigger element to ensure keyboard accessibility without affecting mouse users.
