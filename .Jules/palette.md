...
## 2026-05-10 - Pattern: Missing keyboard support on custom interactive divs
**Learning:** Found a pattern where custom interactive components (like MosaicTile) rely solely on onClick handlers without semantic roles, keyboard event listeners, or focus indicators, making them inaccessible to keyboard users.
**Action:** When adding onClick to div/span elements, always include role="button", tabIndex={0}, an onKeyDown handler for Enter/Space (calling e.preventDefault() for Space), and focus-visible Tailwind classes.
