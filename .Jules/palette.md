...
## 2026-05-06 - Interactive Div Keyboard Accessibility
**Learning:** Found that custom interactive components like MosaicTile use div elements with onClick but lack keyboard navigation and focus indicators, making them inaccessible to keyboard users.
**Action:** When using div as a button, always add role="button", tabIndex={0}, an onKeyDown handler for Enter/Space (with e.preventDefault() for Space), and focus-visible Tailwind classes.
