...
## 2026-06-20 - Native buttons inside Radix Popovers
**Learning:** Custom native HTML `<button>` elements used inside Radix UI `PopoverTrigger` or mapped elements lack default keyboard focus styles and accessible roles like `type='button'` which can impair screen-reader and keyboard accessibility.
**Action:** Always add `type="button"`, `aria-label` (when wrapping icon-only or non-text content), and Tailwind `focus-visible:` utility classes to raw `<button>` tags.
