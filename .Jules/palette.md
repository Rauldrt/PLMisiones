...
## 2026-05-20 - Interactive Custom Divs Keyboard Accessibility
**Learning:** Found custom interactive components (like expanding candidate cards wrapped in `div`s with `onClick`) that lacked keyboard support and visible focus states, making them inaccessible to keyboard users.
**Action:** When adding interactive `onClick` behavior to non-interactive elements like `div` or `span`, always add `role="button"`, `tabIndex={0}`, an `onKeyDown` handler for 'Enter'/'Space' (calling `e.preventDefault()` on Space), and Tailwind `focus-visible` utility classes.
