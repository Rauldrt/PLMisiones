...
## 2026-06-04 - Focus states lost on custom elements within Radix UI triggers
**Learning:** When using custom elements like `<button>` inside Radix UI triggers (like `PopoverTrigger` or `DialogTrigger`) with `asChild`, the default keyboard focus styles are lost.
**Action:** Always manually apply Tailwind CSS focus utilities (`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) and appropriate border-radius classes to the custom trigger element to ensure keyboard accessibility.
