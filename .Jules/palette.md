...
## 2026-07-08 - Focus Management on Native Element Triggers
**Learning:** When using custom elements like `<button>` inside Radix UI triggers (like `PopoverTrigger`) via `asChild`, default keyboard focus styles and attributes may be lost or overridden. Similarly, when replacing non-semantic `<div>` interactive elements with `<button>` for accessibility, we must ensure these native elements receive explicit `type="button"`, `aria-label` (or screen-reader only text), and Tailwind `focus-visible:` utility classes to restore full keyboard accessibility and visible focus state.
**Action:** Always manually apply `focus-visible:outline-none focus-visible:ring-2...`, explicit `type="button"`, and `aria-label` to native elements used as custom triggers or interactive replacements.
