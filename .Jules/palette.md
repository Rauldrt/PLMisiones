...
## 2026-06-18 - Keyboard focus lost on custom Radix UI triggers
**Learning:** When using custom elements like `<button>` inside Radix UI triggers with `asChild`, default keyboard focus styles are often lost or inappropriate because Radix passes focus management to the child but doesn't inject default styles. This creates accessibility barriers for keyboard users navigating triggers like `PopoverTrigger` or `DialogTrigger`.
**Action:** Always manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring`) and appropriate `border-radius` classes to custom trigger elements used with `asChild`. Ensure they also have semantic `type="button"` and `aria-label` if they lack text content.
