...
## 2026-05-03 - Keyboard accessibility on interactive divs
**Learning:** Interactive divs with `onClick` handlers lack native keyboard accessibility by default, which is an accessibility issue.
**Action:** Always add `role="button"`, `tabIndex={0}`, an `onKeyDown` handler for Enter/Space keys (calling `e.preventDefault()` on Space), and `focus-visible` utility classes to ensure keyboard navigation works properly.
