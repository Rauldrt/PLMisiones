...
## 2026-05-30 - Add keyboard focus states to custom Radix UI triggers
**Learning:** When using custom `button` elements as triggers inside Radix UI components (like `PopoverTrigger` or `DialogTrigger`) with `asChild`, they lose default keyboard focus styles. This creates accessibility issues for keyboard navigation as users cannot see which element has focus.
**Action:** Always manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) and appropriate border-radius classes to custom trigger elements to ensure keyboard accessibility and visual correctness.
