...
## 2026-07-07 - Accessible Mosaic Tiles
**Learning:** Converting interactive `<div>` elements to `<button>` elements for accessibility requires adding text alignment (`text-left`) and sizing classes (`w-full`, `h-full`, `block`) to override default browser button styles and prevent layout breakage. It also requires explicit focus styles (`focus-visible:ring-2`) to support keyboard navigation.
**Action:** When converting divs to buttons, always verify layout styles and add focus indicators.
