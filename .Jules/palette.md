...
## 2026-05-02 - Focus styles on Radix UI triggers
**Learning:** When using custom `button` elements as triggers inside Radix UI components (like `PopoverTrigger` or `DialogTrigger`) with `asChild`, they lose default keyboard focus styles, making them invisible to keyboard navigation.
**Action:** Always manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2`) and appropriate border-radius classes to custom trigger elements.
