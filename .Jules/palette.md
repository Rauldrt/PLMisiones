...
## 2024-06-09 - Focus Styles on Radix UI Triggers
**Learning:** When using custom `<button>` elements inside Radix UI triggers (like `PopoverTrigger` or `DialogTrigger`) with `asChild`, default keyboard focus styles are completely lost because the button forwards the ref but doesn't inherit default focus utilities from standard buttons. This breaks keyboard navigation visibility.
**Action:** Always manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`) and matching border-radius classes to the custom trigger element to ensure keyboard accessibility.
