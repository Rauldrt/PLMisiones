...
## 2024-05-18 - Keyboard accessibility on custom interactive elements
**Learning:** Found custom interactive components (`ExpandingCandidateCard`, `MosaicTile`) built with `div` elements lacking keyboard accessibility. While `onClick` handles mouse interactions, keyboard-only users (like those using screen readers or navigating via Tab) cannot trigger them without `role`, `tabIndex`, and specific keydown handlers.
**Action:** When making custom `div` or `span` elements interactive with `onClick`, always add `role="button"`, `tabIndex={0}`, an `onKeyDown` handler listening for 'Enter' or ' ' (Space), call `e.preventDefault()` inside the handler to prevent page scrolling on Space, and add visual focus indicators like Tailwind `focus-visible` utilities.
