...
## 2026-04-19 - Keyboard Accessibility on Clickable Divs
**Learning:** Found that custom interactive components built with `div` elements (like `MosaicTile.tsx`) are often inaccessible to keyboard-only users. They require explicit `role="button"`, `tabIndex={0}`, `onKeyDown` handling for Enter/Space keys, and visible focus states (e.g., Tailwind's `focus-visible` ring).
**Action:** Whenever creating or fixing custom click handlers on non-native interactive elements (`div`, `span`), always implement full keyboard support and focus states to meet WCAG standards.
