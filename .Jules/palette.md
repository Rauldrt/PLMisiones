...
## 2026-05-16 - Radix UI asChild Triggers Focus Issue
**Learning:** When using custom `button` elements as triggers inside Radix UI components (like `PopoverTrigger` or `DialogTrigger`) with `asChild`, they lose their default keyboard focus styles, which degrades keyboard accessibility.
