...
## 2026-07-01 - Use semantic buttons for interactive elements
**Learning:** Found interactive elements built with `<div>` and `onClick` handlers that were inaccessible via keyboard. Using semantic `<button type="button">` elements with appropriate focus styles (`focus-visible:ring-2`) and ARIA attributes (`aria-expanded`, `aria-label`) is essential for keyboard accessibility and screen readers.
**Action:** Always prefer semantic `<button>` tags over `<div>` for clickable elements, and remember to add text alignment and display utilities to override default button styles.
