...
## 2026-06-01 - Focus Management for Custom Radix UI Triggers
**Learning:** When using custom `button` elements inside Radix UI triggers (like `PopoverTrigger` or `DialogTrigger`) with `asChild`, default keyboard focus styles are lost, making them completely inaccessible during keyboard navigation.
**Action:** Always manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) and appropriate border-radius classes to custom trigger elements to preserve focus visibility.
