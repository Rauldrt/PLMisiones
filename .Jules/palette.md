...
## 2026-06-02 - Focus Styles on Custom Triggers
**Learning:** When using custom elements (like `<button>`) inside Radix UI triggers (e.g., `PopoverTrigger`, `DialogTrigger`) with `asChild`, default keyboard focus styles are lost. Also, using `focus:` utilities causes focus rings to inappropriately display when users click elements with a mouse.
**Action:** Manually apply Tailwind CSS focus utilities (`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) and appropriate border-radius classes to custom trigger elements to ensure keyboard accessibility, and prefer `focus-visible:` over `focus:`.
