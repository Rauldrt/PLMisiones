...
## 2026-04-30 - Missing focus styles on custom Radix triggers
**Learning:** When using custom `button` elements as triggers inside Radix UI components like `PopoverTrigger` or `DialogTrigger` with `asChild`, they lose default keyboard focus styles. This creates an accessibility barrier for keyboard-only users as they can't tell when the element has focus.
**Action:** Always manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) and a border-radius class (like `rounded-full` or `rounded-md`) to custom trigger elements to ensure keyboard accessibility.
