...
## 2024-05-27 - Incorrect ARIA label on notification buttons
**Learning:** Adding an `aria-label` to a button that already contains descriptive visible text (like a notification title) completely overrides the internal text for screen readers, hiding crucial context. This is an accessibility anti-pattern.
**Action:** Before adding an `aria-label` to a `<button>`, verify if it only contains an icon or non-descriptive elements. If it contains meaningful text, do not add an `aria-label` that would override it; instead, if necessary, add context using a visually hidden span (`<span className="sr-only"></span>`).
