...
## 2025-02-18 - Missing visual feedback during async operations
**Learning:** Submitting forms (like `NewsForm.tsx`) using React concurrent transitions (`useTransition()`) correctly disables buttons but can lack clear visual feedback indicating activity. Adding an animated spinner icon immediately informs users that their request is being processed.
**Action:** When implementing or fixing disabled states for async operations, include an animated loading spinner (`Icons.Spinner` with `animate-spin` and spacing) alongside the text update to significantly improve perceived performance and feedback.
