...
## 2026-07-05 - Interactive Semantic Elements
**Learning:** Interactive `div` elements lack native keyboard support and semantic meaning. Simply adding `onClick` to a `div` does not make it keyboard accessible or announceable to screen readers.
**Action:** Always convert interactive, non-semantic `<div>` elements into `<button type="button">`. Ensure proper styling by overriding browser defaults using text alignment (`text-left`) and sizing classes (`w-full h-full block`), apply focus styles (`focus-visible:`), and provide an `aria-label` for screen readers.
