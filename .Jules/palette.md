...
## 2026-05-04 - Focus states lost on custom Radix triggers with asChild
**Learning:** When using custom `button` elements as triggers inside Radix UI components (like `PopoverTrigger` or `DialogTrigger`) with the `asChild` prop, the default keyboard focus states (focus rings) are often lost or overridden, causing severe keyboard accessibility regressions.
**Action:** Always manually apply appropriate Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) and explicitly define the border radius (like `rounded-full` or `rounded-md`) directly onto the custom `button` elements being used as triggers.
