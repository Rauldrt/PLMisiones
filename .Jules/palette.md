...
## 2026-06-11 - Keyboard navigation on unstyled Radix triggers and custom buttons
**Learning:** When using unstyled elements like `<button>` or `<div>` inside Radix UI triggers (`asChild`) or as custom interactive elements, default keyboard focus styles are lost or non-existent, breaking keyboard navigation accessibility.
**Action:** Always apply `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2` and an appropriate `border-radius` class to unstyled trigger buttons. Convert clickable `div` elements to semantic `<button type="button">` elements with focus styles and `aria-label`s.
