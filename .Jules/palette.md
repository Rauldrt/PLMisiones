...
## 2024-06-26 - Interactive divs to semantic buttons
**Learning:** Found interactive `div` elements acting as clickable cards in `MosaicTile.tsx` and `BannerContentTabs.tsx` without semantic meaning, making them inaccessible via keyboard. Converting them to `<button type="button">` improves accessibility.
**Action:** Always convert non-semantic `div` interactive elements into `<button type="button">` and explicitly append text alignment (`text-left`) and sizing classes (`w-full`, `h-full`, `block`) to override default browser button styles and prevent layout breakage. Add `focus-visible` utility classes for keyboard focus rings.
