...
## 2024-03-23 - Interactive elements wrapping images/decorations missing labels
**Learning:** This application makes heavy use of `<button>` elements as interactive wrappers around visual content (like mapped `<Image>` components, decorative notification triggers, or full-width cards). These elements often lack text content and fail to provide accessible names, rendering them "unlabeled button" to screen readers.
**Action:** When working with or adding custom interactive wrappers, especially in mapped arrays displaying media or complex layouts, always ensure an `aria-label` is applied to describe the action (e.g., "Ampliar imagen", "Abrir notificaciones", "Ver notificación: [Título]").
