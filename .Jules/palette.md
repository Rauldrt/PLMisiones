...
## 2026-05-27 - Radix UI Trigger keyboard focus
**Learning:** When using custom `button` elements as triggers inside Radix UI components (like `PopoverTrigger` or `DialogTrigger`) with `asChild`, they lose default keyboard focus styles due to Tailwind's preflight. This breaks keyboard accessibility because users cannot see which element has focus.
**Action:** Always manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) and appropriate border-radius classes (like `rounded-full` or `rounded-md`) to custom trigger `button` elements to ensure keyboard accessibility.
