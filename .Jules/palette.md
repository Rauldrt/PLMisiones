...
## 2026-06-03 - Focus Styles on Radix UI Custom Triggers
**Learning:** When wrapping native elements like `<button>` in Radix UI triggers (e.g., `PopoverTrigger`, `DialogTrigger`) using `asChild`, default keyboard focus styles are lost, hindering keyboard navigation accessibility.
**Action:** Always manually apply Tailwind CSS focus utilities (`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) and appropriate border-radius classes to the custom trigger element when using `asChild` to ensure keyboard accessibility.
