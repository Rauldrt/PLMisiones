...
## 2026-06-07 - Focus Styles on Radix UI asChild Triggers
**Learning:** When using custom elements (like `<button>`) inside Radix UI triggers (e.g., `PopoverTrigger`, `DialogTrigger`) with `asChild`, default keyboard focus styles are lost. This makes the elements inaccessible to keyboard users as they cannot see when the element is focused.
**Action:** Manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2`) and appropriate border-radius classes to the custom trigger element to ensure keyboard accessibility.
