...
## 2026-05-25 - Interactive Divs Need Keyboard Support
**Learning:** Interactive `div` elements with `onClick` handlers lack inherent keyboard accessibility, meaning screen reader and keyboard-only users cannot activate them.
**Action:** Always add `role="button"`, `tabIndex={0}`, an `onKeyDown` handler for Enter and Space, and visible focus styles to make custom interactive elements accessible.
