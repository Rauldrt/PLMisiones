...
## 2026-05-24 - Missing focus styles on unstyled buttons inside Radix triggers
**Learning:** When using `<button>` elements as custom triggers inside Radix UI components (like `PopoverTrigger` or `DialogTrigger`) with `asChild`, they lose default keyboard focus styles if they don't use standard button components. This makes keyboard navigation invisible.
**Action:** Always manually apply Tailwind CSS focus utilities (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`) and appropriate border-radius classes to custom trigger elements to ensure keyboard accessibility.
