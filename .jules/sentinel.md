...
## 2026-06-06 - Next.js Server Actions Authentication Bypass
**Vulnerability:** Unauthenticated users could invoke Next.js Server Actions (like `saveNews` or `uploadPublicFilesAction`) because they are public API endpoints by default and do not inherit client-side Firebase authentication.
**Learning:** In Next.js, Server Actions require explicit server-side authorization checks. Without `firebase-admin`, the client must send its ID token via cookies, which can then be validated server-side using the Google Identity Toolkit REST API.
**Prevention:** Always implement and explicitly invoke a `verifyAdmin()` (or similar authorization) check at the very beginning of any sensitive Server Action.
