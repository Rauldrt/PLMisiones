...
## 2026-05-24 - Unauthenticated Server Actions
**Vulnerability:** Next.js Server Actions used to save data and upload files (`src/actions/admin.ts`, `src/actions/gallery.ts`) had no authorization checks, allowing unauthenticated users to overwrite files.
**Learning:** Next.js Server Actions are publicly exposed API endpoints by default. They do not automatically inherit client-side authentication state from Context providers.
**Prevention:** Always implement an explicit server-side authorization check (e.g., validating a session cookie) at the beginning of sensitive Server Actions.
