...
## 2026-04-22 - Prevent Stored XSS via SVG Uploads
**Vulnerability:** Allowed .svg file uploads which can contain embedded malicious JavaScript (<script> tags).
**Learning:** SVG files are XML documents that can execute JavaScript when rendered in a browser. Allowing them in unrestricted media uploads introduces Stored XSS risks.
**Prevention:** Strictly exclude vector formats like .svg from allowed upload extensions; serve uploaded content from an isolated domain with a strict Content Security Policy if SVG support is necessary.
