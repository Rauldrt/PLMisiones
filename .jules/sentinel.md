...
## 2024-04-08 - Stored XSS via SVG File Upload
**Vulnerability:** The public file upload endpoint `uploadPublicFilesAction` allowed the upload of `.svg` files. Since SVG files can contain embedded `<script>` tags, this allowed malicious actors to upload an SVG payload that executes arbitrary JavaScript when loaded in the browser.
**Learning:** Permitting `.svg` uploads on public endpoints without strict sanitization or a separate domain creates a vector for Stored XSS. Browsers execute scripts within SVGs when they are viewed directly or embedded via `<object>` or `<iframe>`.
**Prevention:** Strictly forbid `.svg` in general file upload endpoints, especially when files are served back from the same domain. If SVG upload is absolutely required, it must be rigorously sanitized server-side (e.g., using `DOMPurify` or specialized SVG sanitizers) before storage.
