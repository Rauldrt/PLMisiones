...
## 2026-05-27 - Unauthenticated Next.js Server Actions
**Vulnerability:** Next.js Server Actions (e.g., `saveNews`, `uploadPublicFilesAction`) were publicly exposed without authorization checks, allowing unauthenticated attackers to mutate server JSON files and upload files.
**Learning:** Next.js Server Actions do not automatically inherit client-side authentication state. They act as public API endpoints and require explicit server-side authorization checks for sensitive operations.
**Prevention:** Always implement an authorization wrapper (e.g., `verifyAdmin`) for state-modifying Server Actions. Use a session cookie containing the authentication token (e.g., Firebase ID token via `onIdTokenChanged`) to share client-side auth state with the server securely.
