...
## 2024-06-21 - Unauthenticated Next.js Server Actions
**Vulnerability:** Next.js Server Actions in `src/actions/admin.ts` and `src/actions/gallery.ts` perform sensitive data operations but lacked authentication/authorization checks. Because Server Actions are publicly exposed as API endpoints by default, they could be invoked by any anonymous user, leading to potential data corruption or unauthorized modifications.
**Learning:** Server Actions do not automatically inherit client-side authentication state or route protection (like middleware). Sensitive actions must explicitly invoke a server-side authorization check (e.g., `await verifyAdmin()`) before executing.
**Prevention:** Always implement explicit authentication and authorization validation at the beginning of any Server Action that mutates data or accesses sensitive information.
