...
## 2024-05-27 - Unauthenticated Next.js Server Actions
**Vulnerability:** Next.js Server Actions (e.g., in `src/actions/admin.ts` and `src/actions/gallery.ts`) lacked server-side authentication checks. Because they are publicly exposed as API endpoints by default, anyone could invoke them to modify critical application data and upload files without being logged in.
**Learning:** Client-side route protection (like HOCs checking Firebase state) does not secure Next.js Server Actions. Server Actions must independently re-verify the user's authorization state.
**Prevention:** Always share authentication state via session cookies (using `onIdTokenChanged`) and explicitly invoke an authorization check (e.g., `await verifyAdmin()`) at the top of every sensitive Server Action or data mutation chokepoint.
