...
## 2026-05-21 - Missing Authentication on Server Actions
**Vulnerability:** Next.js Server Actions modifying sensitive data lacked authentication checks, allowing any user to call them directly as API endpoints.
**Learning:** Next.js Server Actions are public endpoints by default. Client-side protection (like HOCs or UI hiding) does not secure the underlying server functions.
**Prevention:** Always implement robust server-side authorization checks (e.g., verifying auth tokens) directly within sensitive Server Actions.
