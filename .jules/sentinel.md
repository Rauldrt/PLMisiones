...
## 2026-06-28 - Unauthenticated Server Actions Bypass Client Auth
**Vulnerability:** Next.js Server Actions used for data modification (e.g., `writeJsonFile`, `uploadPublicFilesAction`) lacked server-side authorization checks, allowing unauthenticated users to modify JSON data or upload files.
**Learning:** Server Actions are publicly exposed API endpoints by default and do not inherit client-side authentication state. Client-side layout protections (like `withAuth`) do not secure the underlying Server Actions.
**Prevention:** Sensitive actions must explicitly invoke a server-side authorization check (e.g., `await verifyAdmin()`) before executing. Share Firebase client auth state with Server Actions by storing the ID token in a secure cookie via `onIdTokenChanged`.
