...
## 2026-06-14 - Unauthenticated Next.js Server Actions
**Vulnerability:** Next.js Server Actions used for CMS data modification and file uploads (`src/actions/admin.ts`, `src/actions/gallery.ts`) did not enforce authentication, allowing any public user to overwrite JSON data files and upload arbitrary media files.
**Learning:** Next.js Server Actions are exposed as public API endpoints by default and do not inherit client-side Firebase authentication state automatically. An explicit server-side check is always required.
**Prevention:** Always implement a server-side authorization check (e.g., verifying a session cookie containing an ID token) at the beginning of sensitive Server Actions, and ensure client authentication state syncs this token to cookies using hooks like `onIdTokenChanged`.
