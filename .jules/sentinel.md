...
## 2026-06-16 - Missing Authentication on Server Actions
**Vulnerability:** Next.js Server Actions (`saveNews`, `uploadPublicFilesAction`) were exposed publicly without authorization checks, allowing unauthenticated attackers to write directly to JSON database files and upload arbitrary media.
**Learning:** Next.js Server Actions are public API endpoints by default. Client-side authentication (e.g., Firebase Auth) does not automatically protect server actions.
**Prevention:** Always implement and explicitly call a server-side authorization check (e.g., `verifyAdmin()`) inside every sensitive Server Action. Share client session state with the server via secure cookies.
