...
## 2026-06-01 - Unauthenticated Next.js Server Actions
**Vulnerability:** Next.js Server Actions (`src/actions/admin.ts`, `src/actions/gallery.ts`) were publicly accessible without authentication. This allowed any user to overwrite critical JSON data files on the server and upload arbitrary media files, leading to complete data compromise.
**Learning:** Next.js Server Actions do not automatically inherit client-side authentication state and are exposed as public API endpoints by default. They must be explicitly secured server-side.
**Prevention:** Share Firebase auth state via session cookies during `onIdTokenChanged` and implement a server-side authorization check (e.g., verifying the token via the Google Identity Toolkit REST API) at the start of all sensitive Server Actions.
