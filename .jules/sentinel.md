...
## 2026-06-18 - Unauthenticated Server Actions
**Vulnerability:** Next.js Server Actions used for writing JSON data and uploading files were missing authentication, allowing unauthenticated users to modify site content and upload arbitrary files.
**Learning:** Next.js Server Actions are public endpoints by default. Even if the UI calling them is behind a client-side auth wrapper, the actions themselves must independently verify authentication on the server.
**Prevention:** Always implement a server-side authentication check (like `verifyAdmin()`) at the very beginning of any Server Action that performs sensitive operations (writes, uploads, deletes). Propagate client-side Firebase auth state to the server via a session cookie set with `onIdTokenChanged`.
