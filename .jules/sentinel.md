...
## 2026-04-25 - Missing Authentication in Server Actions
**Vulnerability:** Next.js Server Actions modifying critical data (JSON files, public gallery) were entirely unauthenticated and exposed as public endpoints.
**Learning:** In Next.js, Server Actions are essentially public API routes unless explicitly protected; UI-level authentication (HOCs/middleware) is insufficient if direct endpoint access is possible.
**Prevention:** Always implement a dedicated authentication check (e.g., `verifyAdmin`) as the first statement inside any Server Action that mutates state or accesses sensitive data.
