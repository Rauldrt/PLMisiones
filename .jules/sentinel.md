...
## 2024-06-15 - Missing Authentication on Server Actions
**Vulnerability:** Next.js Server Actions used for sensitive administrative and AI functions lacked authentication checks, allowing unauthorized users to execute them directly.
**Learning:** Next.js Server Actions are public API endpoints by default and do not automatically inherit client-side authentication state. Sensitive actions must explicitly verify server-side authorization.
**Prevention:** Pass Firebase ID tokens via secure cookies and explicitly invoke a server-side authorization check (e.g., `await verifyAdmin()`) at the start of every sensitive Server Action.
