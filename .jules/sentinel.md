...
## 2026-06-11 - Unauthenticated Server Actions Exposing Write Operations
**Vulnerability:** Next.js Server Actions in `src/actions/admin.ts` and `src/actions/gallery.ts` were publicly exposed, allowing unauthenticated users to overwrite all JSON data files and upload files to the public directory.
**Learning:** Next.js Server Actions are public API endpoints by default. They do not automatically inherit client-side authentication state or route protections.
**Prevention:** Always explicitly invoke a server-side authorization check (e.g., `await verifyAdmin()`) at the very beginning of sensitive Server Actions. To share Firebase client auth state with the server without the admin SDK, store the ID token in a cookie via `onIdTokenChanged`.
