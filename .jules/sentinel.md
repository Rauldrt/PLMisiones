...
## 2026-06-30 - Missing Authentication on Server Actions
**Vulnerability:** Next.js Server Actions modifying global JSON data files (`src/actions/admin.ts`, `src/actions/gallery.ts`) were publicly exposed without authentication checks.
**Learning:** Server Actions are publicly exposed API endpoints by default and do not inherit client-side Firebase auth state automatically. A malicious user could send requests directly to these actions and arbitrarily modify the website's data and structure.
**Prevention:** Explicitly implement a server-side authorization check (e.g., `await verifyAdmin()`) inside every sensitive Server Action. Share client auth state by storing the Firebase ID token in a cookie via `onIdTokenChanged` and verifying it using the Google Identity Toolkit REST API.
