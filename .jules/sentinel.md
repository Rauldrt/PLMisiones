...
## 2026-05-16 - Missing Authentication on Server Actions
**Vulnerability:** Next.js Server Actions were completely unauthenticated, allowing any visitor to invoke them and modify data via POST requests.
**Learning:** Next.js Server Actions execute on the server but are publicly exposed as API endpoints by default. Firebase client-side auth state does not automatically protect server actions.
**Prevention:** Always implement explicit authentication and authorization checks (e.g., `await verifyAdmin()` validating a session cookie) at the very beginning of every server action handling sensitive operations.
