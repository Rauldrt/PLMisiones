...
## 2026-06-19 - Unauthenticated Server Actions for data mutation
**Vulnerability:** Critical Next.js Server Actions modifying data (e.g., `saveNews`, `uploadPublicFilesAction`) and triggering AI flows were public without authentication, allowing anyone to modify site content.
**Learning:** Next.js Server Actions are public API endpoints by default and do not inherit client-side Firebase authentication state without explicitly sharing and verifying it server-side.
**Prevention:** Always verify authorization at the beginning of Server Actions using server-side validated session tokens (e.g., verifying Firebase ID tokens from a cookie).
