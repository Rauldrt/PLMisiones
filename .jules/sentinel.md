...
## 2026-05-22 - Unauthenticated Server Actions
**Vulnerability:** Next.js Server Actions modifying files and data (like `saveNews`, `uploadPublicFilesAction`) lacked explicit server-side authentication checks, exposing sensitive endpoints to unauthenticated attackers.
**Learning:** Server Actions are publicly exposed API endpoints by default and do not inherit client-side auth state. Without explicit `verifyAdmin()` checks reading session cookies, anyone can execute them.
**Prevention:** Always implement and invoke a server-side authorization check (e.g., verifying a Firebase ID token from a session cookie via the Identity Toolkit REST API) at the start of any sensitive Server Action.
