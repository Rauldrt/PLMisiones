...
## 2026-04-13 - TOCTOU SSRF in Genkit URL Fetcher
**Vulnerability:** The `generateNewsContent` AI flow fetched URLs safely by resolving DNS via `dns.promises.lookup` to block private network IPs. However, this is vulnerable to Time-of-Check to Time-of-Use (TOCTOU) DNS Rebinding attacks. An attacker could respond with a public IP during the lookup, and then change the DNS record to a private IP when the actual `fetch` happens.
**Learning:** Mitigating SSRF in Node.js using native `fetch` is complex because checking the IP beforehand is prone to TOCTOU, and manually resolving the IP and swapping the hostname breaks HTTPS Server Name Indication (SNI).
**Prevention:** Use the `@atproto-labs/fetch-node` package's `safeFetchWrap`. It prevents SSRF at the socket connection layer by checking the connected IP addresses, successfully avoiding TOCTOU without breaking TLS/SNI.
