...
## 2026-06-09 - Unauthenticated Server Actions Bypass Frontend Security
**Vulnerability:** Next.js Server Actions modifying the filesystem and uploading files lacked explicit authorization checks, trusting the frontend HOC router protections instead. This allows bypassing the UI entirely via direct POST requests to the Action endpoint.
**Learning:** Next.js Server Actions are public API endpoints by default. Frontend authentication state does not automatically transfer to Server Actions without explicitly passing session context (e.g., via cookies).
**Prevention:** Always implement an explicit, server-side authorization check (e.g., `await verifyAdmin()` validating a session cookie) at the beginning of any Server Action that mutates data or performs sensitive operations.
