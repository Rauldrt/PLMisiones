...
## 2026-05-29 - Server Action Authentication Bypass
**Vulnerability:** Next.js Server Actions modifying critical data and uploading files had no server-side authentication, allowing any unauthenticated user to invoke them.
**Learning:** Server Actions are exposed as public API endpoints by default and do not inherently know about client-side Firebase Auth state.
**Prevention:** Use cookie-based sessions synced with `onIdTokenChanged` and manually enforce a `verifyAdmin()` check inside sensitive Server Actions before executing any logic.
