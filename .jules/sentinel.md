...
## 2026-05-18 - Securing Next.js Server Actions
**Vulnerability:** Server Actions in Next.js were publicly exposed by default without explicit server-side authentication, allowing unauthenticated users to execute sensitive operations like file writes.
**Learning:** Next.js Server Actions are essentially public API endpoints and do not automatically inherit client-side Firebase authentication state unless explicitly passed or synced via cookies.
**Prevention:** Always implement explicit server-side authorization checks (e.g., verifyAdmin()) at the beginning of sensitive Server Actions, and sync client auth state to a session cookie.
