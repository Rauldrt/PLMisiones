...
## 2026-05-17 - Next.js Server Actions Authorization Bypass
**Vulnerability:** Next.js Server Actions (e.g., `src/actions/admin.ts`, `src/actions/gallery.ts`) lacked authentication checks, allowing unauthenticated users to invoke them and modify server state directly (e.g. overwriting files).
**Learning:** Server Actions are exposed as public API endpoints and do not inherit client-side authentication automatically. They must explicitly verify authentication on the server-side before proceeding.
**Prevention:** Always implement an explicit authorization check like `await verifyAdmin()` at the beginning of sensitive Server Actions, using a token passed via secure HTTP cookies.
