...
## 2024-05-24 - Node test runner with TSX
**Learning:** When using `node --experimental-strip-types` for benchmarking or isolated scripts, it does not support JSX syntax (it will throw `ERR_INVALID_TYPESCRIPT_SYNTAX` on JSX tags).
**Action:** Do not use `node --experimental-strip-types` to render React components containing JSX syntax.
