...
## 2026-07-06 - Optimize Cache Invalidation
**Learning:** Calling `revalidatePath` inside a loop for dynamic paths creates redundant O(N) operations, and using `revalidatePath('/*')` is invalid for global invalidation in Next.js.
**Action:** Deduplicate paths using a `Set` before calling `revalidatePath` in loops, and use `revalidatePath('/', 'layout')` for global invalidations.
