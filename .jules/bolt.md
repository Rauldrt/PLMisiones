...
## 2024-05-24 - Deduplicate Cache Invalidation Paths
**Learning:** In Next.js Server Actions, iterating through items to invalidate caches can lead to redundant O(N) calls to `revalidatePath` if multiple items target the same path.
**Action:** Always deduplicate an array of paths using a `Set` before looping and calling `revalidatePath` (e.g., `Array.from(new Set(items.map(i => i.path))).forEach(path => revalidatePath(path))`) to prevent unnecessary invalidation operations.
