...
## 2024-05-24 - Global Cache Invalidation with `revalidatePath`
**Learning:** In Next.js Server Actions, `revalidatePath('/*')` is invalid for global cache invalidation. It does not revalidate all pages as might be expected.
**Action:** To revalidate the entire application globally, use `revalidatePath('/', 'layout')` instead.
