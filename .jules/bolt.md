...
## 2026-05-13 - Deduplicate cache invalidation paths
**Learning:** Iterating over items to call `revalidatePath` can lead to redundant, expensive O(N) cache invalidation operations if the list contains duplicate paths.
**Action:** Deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) before invalidating the cache to minimize redundant calls.
