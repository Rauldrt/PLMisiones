...
## 2026-05-02 - Deduplicate redundant revalidatePath calls
**Learning:** Calling `revalidatePath` multiple times for the same path inside a loop where items might share paths creates redundant cache invalidation operations.
**Action:** Deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths))`) before iterating to ensure each unique path is revalidated only once.
