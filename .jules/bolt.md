...
## 2026-05-09 - Replace DOM Polling with MutationObserver
**Learning:** Continuously polling the DOM with `setInterval` to detect asynchronously injected content causes unnecessary main-thread blocking and layout thrashing.
**Action:** Use a `MutationObserver` configured with `childList: true` and `subtree: true` to watch for injected nodes. Use `mutations.some(m => m.addedNodes.length > 0)` to quickly filter relevant events before executing expensive document-wide searches.
