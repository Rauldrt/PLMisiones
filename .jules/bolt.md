...
## 2026-04-13 - Deduplicate Cache Invalidations in Loops
**Learning:** Calling `revalidatePath(path)` multiple times for the same path in a loop causes redundant operations that scale poorly.
**Action:** When invalidating cache based on an array of items, always deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths))`) before iterating to prevent redundant invalidation cycles.
