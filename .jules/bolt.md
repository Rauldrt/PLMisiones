...
## 2026-05-27 - Replace setInterval with MutationObserver for DOM queries
**Learning:** Continuously polling the DOM with `setInterval` and `document.querySelector` causes unnecessary main-thread blocking, layout thrashing, and excessive battery usage, especially in client-heavy Next.js applications.
**Action:** When watching for asynchronously injected DOM content (like third-party embeds), always use a `MutationObserver` that specifically traverses `mutation.addedNodes` instead of running full-document queries on every tick.
