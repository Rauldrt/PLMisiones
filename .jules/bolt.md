...
## 2026-06-12 - Deduplicate revalidatePath calls
**Learning:** When invoking `revalidatePath` within a loop based on an array of items, if multiple items share the same route, it triggers redundant cache invalidations blocking the thread unnecessarily.
**Action:** Always map the items to their target paths and deduplicate them using a `Set` before looping over them to call `revalidatePath`.
