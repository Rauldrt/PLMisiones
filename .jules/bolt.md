...
## 2026-05-11 - Deduplicate revalidatePath calls in loops
**Learning:** Calling revalidatePath in a loop on potentially duplicate paths creates redundant O(N) cache invalidation operations.
**Action:** When iterating over items where paths may duplicate, always deduplicate using a Set (e.g., Array.from(new Set(paths))) before invalidating.
