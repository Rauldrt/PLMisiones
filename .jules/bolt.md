...
## 2024-05-25 - Deduplicate revalidatePath calls using a Set
**Learning:** Calling revalidatePath for an array of items without deduplication can trigger redundant, expensive cache invalidations if multiple items share the same path.
**Action:** When updating or saving an array of items that trigger revalidation, map the items to their paths and wrap them in a Set (e.g., Array.from(new Set(paths))). Iterate over the unique paths to perform revalidation efficiently.
