...
## 2026-05-22 - Replaced setInterval polling with MutationObserver
**Learning:** Continuously polling the DOM for asynchronously injected third-party elements (like embeds) using `setInterval` causes unnecessary main-thread blocking, repeated layout calculations, and general performance degradation.
**Action:** Always use a `MutationObserver` with `childList: true, subtree: true` to listen for DOM changes, avoiding continuous polling and thrashing.
