...
## 2026-05-17 - Deduplicate revalidatePath calls
**Learning:** Iterating over items and calling `revalidatePath` can lead to redundant O(N) cache invalidation operations if paths duplicate.
**Action:** Deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths))`) before invalidating to ensure O(1) operations per unique path.
