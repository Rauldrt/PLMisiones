...
## 2026-06-07 - Deduplicate paths in loops before revalidation
**Learning:** Calling `revalidatePath` individually inside a loop when updating multiple items that share the same path (e.g., page headers or sections) causes redundant O(N) invalidation operations, which can slow down the action execution.
**Action:** Use a `Set` to deduplicate the array of paths before looping over it with `revalidatePath` to avoid redundant cache invalidations (e.g., `Array.from(new Set(items.map(item => item.path))).forEach(path => revalidatePath(path));`).
