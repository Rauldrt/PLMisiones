...
## 2026-06-04 - Replace setInterval with MutationObserver for DOM Detection
**Learning:** Using `setInterval` with `document.querySelector` to detect asynchronously loaded elements blocks the main thread every tick and drains battery. Furthermore, full document queries inside `MutationObserver` callbacks cause severe regressions.
**Action:** Always use `MutationObserver` to detect DOM insertions, but restrict DOM queries to the inserted subtrees by checking `node instanceof HTMLElement` and querying within `mutation.addedNodes`.
