...
## 2026-06-18 - Deduplicate revalidatePath calls
**Learning:** Calling `revalidatePath` within a loop for items that share the same path results in redundant, blocking cache invalidation operations (O(N) time complexity).
**Action:** Always deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) before iterating to trigger revalidations, ensuring each unique path is only invalidated once.
