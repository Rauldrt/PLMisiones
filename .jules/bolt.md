...
## 2026-06-14 - Deduplicate revalidatePath calls
**Learning:** Calling revalidatePath in a loop on an array that may contain duplicate paths creates redundant O(N) cache invalidation operations, causing unnecessary performance bottlenecks.
**Action:** When revalidatePath must be called in a loop, always map the paths and deduplicate them using a Set before iteration to ensure each path is only revalidated once.
