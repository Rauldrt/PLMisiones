...
## 2026-05-19 - Deduplicate revalidatePath calls
**Learning:** Calling revalidatePath in a loop without deduplication causes redundant O(N) cache invalidation operations when items share the same path.
**Action:** Always deduplicate paths using a Set (e.g., `Array.from(new Set(paths))`) before iterating to revalidate.
