...
## 2026-06-05 - Deduplicate revalidatePath calls
**Learning:** Calling `revalidatePath` in a loop over an array that may contain duplicate paths causes redundant cache invalidation operations, needlessly blocking the server thread.
**Action:** When invalidating multiple paths based on items in an array, use a `Set` (e.g., `Array.from(new Set(paths))`) to deduplicate the paths before iterating and calling `revalidatePath`.
