...
## 2026-07-05 - Deduplicate Paths for Cache Invalidation
**Learning:** When invalidating Next.js caches dynamically in a loop based on arrays of data, calling `revalidatePath` sequentially on overlapping or redundant paths creates unnecessary execution overhead and delays.
**Action:** Deduplicate the target paths using a `Set` (e.g., `Array.from(new Set(items.map(i => i.path))).forEach(path => revalidatePath(path))`) to prevent redundant cache invalidation operations.
