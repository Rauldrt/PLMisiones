...
## 2026-05-20 - Replace DOM Polling with MutationObserver
**Learning:** Continuously polling the DOM with `setInterval` for asynchronously injected content causes unnecessary layout thrashing and main-thread blocking.
**Action:** When watching for asynchronously injected DOM content, use a `MutationObserver` instead of `setInterval`. Check `mutations.some(m => m.addedNodes.length > 0)` before executing expensive document-wide searches to maximize performance.
