...
## 2026-05-21 - Deduplicate revalidatePath calls
**Learning:** Calling revalidatePath inside a loop where paths may duplicate creates redundant O(N) cache invalidation operations.
**Action:** Deduplicate the paths using a Set (e.g., Array.from(new Set(paths)).forEach(...)) before calling revalidatePath to prevent redundant invalidations.
