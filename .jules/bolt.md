...
## 2026-05-29 - Deduplicate revalidatePath calls
**Learning:** Calling revalidatePath inside a loop where paths may duplicate causes redundant O(N) cache invalidations.
**Action:** Deduplicate the paths using a Set (e.g., Array.from(new Set(paths)).forEach(...)) before invalidation to prevent redundant operations.
