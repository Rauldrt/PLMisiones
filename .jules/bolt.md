...
## 2026-05-23 - Deduplicate revalidatePath calls
**Learning:** Calling `revalidatePath` in a loop where items might map to the same path causes redundant, expensive cache invalidation operations.
**Action:** When invalidating multiple dynamic paths, always deduplicate the paths first using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) to minimize cache invalidation overhead.
