...
## 2025-02-24 - Deduplicate revalidatePath calls and fix global invalidation
**Learning:** In Next.js Server Actions, `revalidatePath('/*')` is invalid for global cache invalidation and silently fails to clear caches effectively. Additionally, calling `revalidatePath` within loops over items that share paths can lead to redundant O(N) operations.
**Action:** Use `revalidatePath('/', 'layout')` for true global cache invalidation. Deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) before looping to minimize performance bottlenecks in cache invalidation.
