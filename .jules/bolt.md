...
## 2024-05-25 - Deduplicate paths in revalidatePath loops
**Learning:** When calling `revalidatePath` within a loop over an array of items, duplicate paths can trigger redundant O(N) cache invalidation operations, leading to performance bottlenecks.
**Action:** Deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) before executing the invalidation loop to avoid redundant calls.
