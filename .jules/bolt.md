...
## 2026-06-02 - Replace interval polling with MutationObserver
**Learning:** Continuous DOM polling via `setInterval` and `document.querySelector` causes unnecessary main-thread CPU usage, even when nothing changes, leading to poor performance and battery drain.
**Action:** Use a `MutationObserver` to detect asynchronously injected DOM elements. To avoid performance regressions on unrelated mutations, specifically traverse `mutation.addedNodes` using `node.querySelector` rather than falling back to a full document-wide query.
