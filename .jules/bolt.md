...
## 2025-02-18 - revalidatePath('/*', 'layout') Anti-pattern
**Learning:** In Next.js App Router, using `revalidatePath('/*')` is an invalid wildcard pattern. To invalidate global components (like footers or navigation), you must use `revalidatePath('/', 'layout')`. However, do NOT blindly replace O(N) loops of `revalidatePath(item.path)` with `revalidatePath('/', 'layout')` when dealing with granular path updates. While it's O(1) in terms of API calls, `revalidatePath('/', 'layout')` triggers a complete site-wide cache purge, forcing every single page to re-render on the next request. This is a massive performance regression and degrades TTFB (Time to First Byte) compared to a loop of 5-10 specific revalidations.
**Action:** Use `revalidatePath('/', 'layout')` ONLY for genuinely global updates (like a footer change). For granular array updates, stick to the O(N) loop of `revalidatePath(path)`.
