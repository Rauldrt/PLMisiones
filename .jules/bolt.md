...
## 2026-07-02 - Optimize Next.js Cache Invalidation
**Learning:** Using `revalidatePath('/*')` is invalid for global invalidation and silently fails, while looping through non-deduplicated arrays to call `revalidatePath` causes redundant O(N) operations.
**Action:** Use `revalidatePath('/', 'layout')` for full application revalidation and deduplicate array maps with `Set` before looping cache invalidations.
