...
## 2026-06-30 - Deduplicate paths for revalidatePath
**Learning:** Calling `revalidatePath` multiple times for the same path in a loop can cause unnecessary cache invalidation operations.
**Action:** Always deduplicate paths using a `Set` before looping over them to call `revalidatePath`. Also, `revalidatePath('/*')` is invalid; use `revalidatePath('/', 'layout')` for global invalidation.
