...
## 2026-05-24 - Use MutationObserver over setInterval for async DOM injection
**Learning:** Continuously polling the DOM with `setInterval` for asynchronously injected content like third-party embeds blocks the main thread and creates unnecessary processing overhead.
**Action:** When watching for asynchronously injected DOM content, use a `MutationObserver` instead of `setInterval`. This prevents unnecessary main-thread blocking and layout thrashing.
