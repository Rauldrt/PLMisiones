...
## 2026-04-26 - Deduplicate paths in loops for revalidatePath
**Learning:** Calling `revalidatePath` within a loop without deduplication can trigger redundant cache invalidation operations for the same paths, causing an O(N) performance bottleneck.
**Action:** Deduplicate the collection of paths using a `Set` (e.g., `Array.from(new Set(paths))`) before applying iterative cache invalidation.
