...
## 2026-06-28 - Use revalidatePath('/', 'layout') for global cache invalidation
**Learning:** In Next.js Server Actions, `revalidatePath('/*')` is invalid for global cache invalidation and can silently fail or miss paths. Furthermore, iterating arrays to call `revalidatePath` on dynamic paths can lead to redundant O(N) operations.
**Action:** To globally invalidate the Next.js cache, always use `revalidatePath('/', 'layout')`. For array iterations, always deduplicate paths using a `Set` before revalidating to reduce cache invalidation overhead.
