...
## 2026-05-03 - Deduplicating paths before batch `revalidatePath`
**Learning:** When invalidating cache across a list of items using `revalidatePath`, some items may share the same path. Executing `revalidatePath` iteratively without deduplication causes redundant O(N) cache invalidation calls which degrades server performance unnecessarily.
**Action:** Optimize loops that call `revalidatePath` by converting the list of paths to a `Set` first to deduplicate them, ensuring each path is revalidated only once (e.g., `Array.from(new Set(paths)).forEach(path => revalidatePath(path))`).
