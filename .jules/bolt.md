...
## 2026-05-30 - Deduplicate revalidatePath calls in loops
**Learning:** Calling `revalidatePath` within a loop on items that might share the same path results in redundant cache invalidation operations, which can degrade server performance during bulk updates.
**Action:** Always extract and deduplicate paths using a `Set` before calling `revalidatePath` in a loop to ensure each unique path is invalidated only once.
