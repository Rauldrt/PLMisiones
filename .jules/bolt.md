...
## 2026-05-18 - Deduplicate revalidatePath calls
**Learning:** Calling `revalidatePath` within a loop without deduplication results in redundant O(N) cache invalidation operations, which scales poorly and wastes processing time if paths duplicate.
**Action:** Deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) to prevent redundant invalidations.
