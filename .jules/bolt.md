...
## 2026-04-20 - Deduplicate paths for revalidatePath
**Learning:** Calling `revalidatePath` inside a loop can trigger redundant cache invalidation operations if the items share the same path. This causes an unnecessary O(N) performance bottleneck when O(1) per distinct path is sufficient.
**Action:** When invalidating cache across a list of items using `revalidatePath`, always extract and deduplicate the paths first (e.g., using `Array.from(new Set(items.map(item => item.path)))`) before performing the invalidation loop.
