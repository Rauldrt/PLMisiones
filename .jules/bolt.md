...
## 2026-07-04 - Deduplicate Cache Invalidations
**Learning:** Calling `revalidatePath` repeatedly on the same path inside a loop based on arrays of data causes redundant cache invalidation operations, which wastes server resources and degrades performance.
**Action:** When invalidating Next.js caches dynamically based on arrays of data, always deduplicate the target paths using a `Set` (e.g., `Array.from(new Set(items.map(i => i.path))).forEach(path => revalidatePath(path))`) to prevent redundant cache invalidation calls.
