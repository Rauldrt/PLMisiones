...
## 2026-06-24 - Deduplicate revalidatePath calls
**Learning:** When calling `revalidatePath` within a loop where paths may duplicate, it creates redundant O(N) cache invalidation operations that block the main thread unnecessarily.
**Action:** Deduplicate the paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) to prevent redundant cache invalidation operations.
