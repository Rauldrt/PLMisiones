...
## 2026-05-08 - Replaced setInterval with MutationObserver for Third-Party Embeds
**Learning:** Continuously polling the DOM with setInterval to check for dynamically injected elements (like third-party embeds) unnecessarily blocks the main thread and can cause layout thrashing.
**Action:** Use a MutationObserver to watch for added DOM nodes and use mutations.some(m => m.addedNodes.length > 0) to check if any relevant nodes were added before executing expensive document-wide searches.
