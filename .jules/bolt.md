...
## 2024-05-24 - Deduplicate and correct global revalidatePath
**Learning:** In Next.js, `revalidatePath('/*')` is invalid for global cache invalidation; `revalidatePath('/', 'layout')` must be used instead. Furthermore, looping over duplicate paths for revalidation creates redundant O(N) operations.
**Action:** Use `revalidatePath('/', 'layout')` for global revalidations, and deduplicate paths using a `Set` before looping over them for specific path revalidations.
