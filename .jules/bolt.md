...
## 2026-06-08 - Use MutationObserver over Polling for DOM Injections
**Learning:** Polling the DOM using `setInterval` and `document.querySelector` to detect dynamically injected elements causes continuous CPU wakeups and layout thrashing even when the app is idle.
**Action:** Replace DOM polling intervals with `MutationObserver` watching `childList: true, subtree: true`. Safely check `node instanceof HTMLElement` on `addedNodes` and debounce the processing to handle rapid consecutive mutations efficiently.
