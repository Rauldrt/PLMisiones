...
## 2026-05-07 - Deduplicate paths in revalidatePath loops
**Learning:** When using `revalidatePath` inside a loop for dynamic paths, iterating over an array that contains duplicate paths leads to redundant O(N) cache invalidation operations.
**Action:** When optimizing `revalidatePath` calls within a loop where paths may duplicate, deduplicate the paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) to prevent redundant O(N) cache invalidation operations.
