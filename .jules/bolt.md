...
## 2026-06-26 - Deduplicate Paths for revalidatePath
**Learning:** Calling `revalidatePath` within a loop on items that share the same path results in redundant cache invalidation calls, which creates unnecessary overhead.
**Action:** When updating multiple items that may reside on the same route, extract the paths, deduplicate them using a `Set`, and then map over the unique paths to call `revalidatePath`.
