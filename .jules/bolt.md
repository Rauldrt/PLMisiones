...
## 2026-06-15 - Deduplicating paths before revalidatePath
**Learning:** Calling `revalidatePath` in a loop where paths might be duplicated results in redundant O(N) cache invalidation operations, which wastes server resources during data updates.
**Action:** Always deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) when batch-invalidating multiple paths dynamically from an array.
