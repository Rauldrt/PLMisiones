...
## 2026-05-25 - Use MutationObserver over setInterval for DOM polling
**Learning:** Continuously polling the DOM with `setInterval` causes unnecessary main-thread blocking and layout thrashing, even when no changes occur.
**Action:** When watching for asynchronously injected DOM content (e.g., third-party embeds), use a `MutationObserver` instead of `setInterval` to react to changes only when they happen.
