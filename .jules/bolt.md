...
## 2026-07-08 - Correct Global Cache Invalidation
**Learning:** Next.js Server Actions `revalidatePath('/*')` is invalid for global cache invalidation. Additionally, invalidating paths inside a loop from user input arrays creates redundant O(N) operations.
**Action:** Use `revalidatePath('/', 'layout')` to globally invalidate all cache. When invalidating paths from arrays, deduplicate paths first using `Set` to ensure each path is only revalidated once.
