...
## 2026-06-03 - Deduplicate paths before calling revalidatePath in loops
**Learning:** Iterating over an array and calling revalidatePath on each item can result in redundant O(N) cache invalidation operations if multiple items share the same path, causing unnecessary server load.
**Action:** When calling revalidatePath inside loops where paths may duplicate, always deduplicate the paths using a Set (e.g., Array.from(new Set(paths)).forEach(...)).
