...
## 2026-06-06 - Deduplicate revalidatePath calls
**Learning:** Calling `revalidatePath` within a loop where paths may duplicate results in redundant O(N) cache invalidation operations.
**Action:** Deduplicate paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) before iterating and calling `revalidatePath` to prevent unnecessary performance degradation during cache invalidation.
