...
## 2026-05-26 - Deduplicate revalidatePath calls in loops
**Learning:** Calling `revalidatePath` within a loop where paths may duplicate creates redundant O(N) cache invalidation operations.
**Action:** Deduplicate the paths using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) to prevent redundant cache invalidation.
