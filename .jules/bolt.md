...
## 2026-06-17 - Optimizing Global and Repeated Cache Invalidations
**Learning:** Using `revalidatePath('/*')` is invalid for global cache invalidation in Next.js Server Actions and fails to clear the cache properly. Additionally, calling `revalidatePath` within loops over items that share the same path triggers redundant, expensive O(N) cache invalidation operations.
**Action:** Use `revalidatePath('/', 'layout')` to correctly and efficiently revalidate the entire application globally. When invalidating paths from a list, always deduplicate them using a `Set` (e.g., `Array.from(new Set(paths)).forEach(...)`) to minimize revalidation calls.
