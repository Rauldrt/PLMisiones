...
## $(date +%Y-%m-%d) - Optimize Client-Side Search with useMemo
**Learning:** Performing client-side array filtering inside a render loop without `useMemo` forces an O(N) string operation (like `toLowerCase().includes()`) on every component re-render, even for unrelated state changes. Furthermore, calling `searchTerm.toLowerCase()` inside the filter loop re-evaluates the same string for every item in the array, amplifying the performance cost.
**Action:** Always wrap expensive client-side array transformations (like search filters) in a `useMemo` hook. When filtering against a string, compute the lowercase search term once outside the `.filter` loop and reuse the cached variable to minimize redundant string operations.
