                ...
                <UiAccordionItem key={item.id} value={item.id}>
                    <div className="flex justify-between items-center w-full pr-4">
                      ...
                      <div className="flex gap-2 items-center" onClick={(e) => e.stopPropagation()}>
                          <Button aria-label="Subir ítem" variant="ghost" size="icon" onClick={() => moveItem(index, 'up')} disabled={index === 0}>
                              <Icons.ChevronUp className="w-4 h-4"/>
                          </Button>
                            <Button aria-label="Bajar ítem" variant="ghost" size="icon" onClick={() => moveItem(index, 'down')} disabled={index === items.length - 1}>
                              <Icons.ChevronDown className="w-4 h-4"/>
                          </Button>
                          <Button aria-label="Eliminar ítem" variant="destructive" size="icon" onClick={() => removeItem(item.id)}><Icons.Trash className="w-4 h-4"/></Button>
                      </div>
                    </div>
