...
export function ExpandingCandidateCard({ candidate, isExpanded, onClick }: ExpandingCandidateCardProps) {
  ...
  return (
    <button
      ...
      aria-label={isExpanded ? `Cerrar detalles de ${candidate.name}` : `Ver detalles de ${candidate.name}`}
      className={cn(
        'group relative w-full cursor-pointer overflow-hidden rounded-lg text-left block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
        transitionClass,
        easeClass,
        isExpanded 
