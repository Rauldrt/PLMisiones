                  ...
                  <Card key={item.id} className="overflow-hidden">
                    {item.imageUrl && (
                      <DialogTrigger asChild>
                         <button
                            className="relative h-96 w-full cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-lg"
                            onClick={() => handleImageClick(item.imageUrl!)}
                            aria-label={item.title ? `Ampliar imagen de ${item.title}` : "Ampliar imagen de notificación"}
                         >
                            <Image
                                src={item.imageUrl}
                                alt={item.title}
