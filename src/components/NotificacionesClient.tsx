                  ...
                  <Card key={item.id} className="overflow-hidden">
                    {item.imageUrl && (
                      <DialogTrigger asChild>
                         <button
                            className={cn(
                                "relative h-96 w-full cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2",
                                hasTextContent ? "rounded-t-lg" : "rounded-lg"
                            )}
                            onClick={() => handleImageClick(item.imageUrl!)}
                            aria-label={item.title ? `Ampliar imagen: ${item.title}` : "Ampliar imagen"}
                         >
                            <Image
                                src={item.imageUrl}
