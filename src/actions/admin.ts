...
export async function saveFooterContent(item: FooterContent) {
    ...
    // ⚡ Bolt: Use 'layout' for valid global cache invalidation in Next.js instead of '/*'
...
export async function savePageHeaders(items: PageHeader[]) {
    ...
    // ⚡ Bolt: Deduplicate target paths using a Set to prevent redundant O(N) cache invalidation operations
