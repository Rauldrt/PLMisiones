...
export async function saveFooterContent(item: FooterContent) {
    ...
    // ⚡ Bolt: Use revalidatePath('/', 'layout') for correct and efficient global cache invalidation
...
export async function savePageHeaders(items: PageHeader[]) {
    ...
    // ⚡ Bolt: Deduplicate paths using a Set to prevent redundant O(N) cache invalidation operations
