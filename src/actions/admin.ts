...
export async function saveFooterContent(item: FooterContent) {
    ...
    revalidatePath('/', 'layout'); // ⚡ Bolt: Use layout revalidation for global cache instead of invalid '/*'
...
export async function savePageHeaders(items: PageHeader[]) {
    ...
    // ⚡ Bolt: Deduplicate paths to avoid O(N) redundant cache invalidations
