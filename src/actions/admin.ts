...
export async function saveFooterContent(item: FooterContent) {
    ...
    // ⚡ Bolt: Use revalidatePath('/', 'layout') for global cache invalidation instead of invalid '/*'
...
export async function savePageHeaders(items: PageHeader[]) {
    ...
    // ⚡ Bolt: Deduplicate paths to prevent redundant O(N) cache invalidation operations
