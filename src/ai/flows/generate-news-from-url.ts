...
const safeFetch = safeFetchWrap({ allowImplicitRedirect: true });
...
const fetchAndParseUrlTool = ai.defineTool(
  ...
  async ({ url }) => {
    try {
      // SECURITY: Protect against SSRF using @atproto-labs/fetch-node
      // This provides TOCTOU-safe fetching and follows redirects safely.
